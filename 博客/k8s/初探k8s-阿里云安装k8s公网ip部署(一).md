...
![v2-7641945507f855e9bcd56a91aba26742_720w](v2-7641945507f855e9bcd56a91aba26742_720w.jpg)
...
# 设置主机名
hostnamectl set-hostname kubernetes-nodex
...
从主节点机器上copy kubeadm配置文件`/etc/kubernetes/admin.conf`到从节点机器，复制文件到`$HOME/.kube/`文件下，文件名修改为`config`

> 下载上传需要先安装  lrzsz
>
> ubuntu 命令 apt install lrzsz
>
> centos及其他系统自行百度
...
# 创建.kube文件夹 root账户$HOME地址为/root/
mkdir $HOME/.kube

# 复制文件到.kube文件夹下并修改文件名为config
cp -i {admin.config文件上传地址} $HOME/.kube/config
```
...
izuf65pz7210orjwg7jonsz   Ready    <none>   114m   v1.18.4
izuf6g67k0g3591un0lm01z   Ready    <none>   114m   v1.18.4
izuf6it7xbkq67r44829wnz   Ready    <none>   122m   v1.18.4
kubernetes-master         Ready    master   140m   v1.18.3
```
...
```shell
root@kubernetes-master:/usr/local/kubernetes/service# kubectl get nodes
# 日志输出
NAME                STATUS   ROLES    AGE     VERSION
kubernetes-master   Ready    master   6d2h    v1.18.3
kubernetes-node1    Ready    <none>   5d23h   v1.18.4
kubernetes-node2    Ready    <none>   5d23h   v1.18.4
kubernetes-node3    Ready    <none>   5d22h   v1.18.4
...
删除节点前，使用该命令将节点上的Pod应用服务驱逐到其它节点上。

```shell
root@kubernetes-master:/usr/local/kubernetes/service# kubectl drain kubernetes-node3 --delete-local-data --force --ignore-daemonsets
# 输出内容
node/kubernetes-node3 cordoned
WARNING: ignoring DaemonSet-managed Pods: kube-system/calico-node-jjq2r, kube-system/kube-proxy-hz88x
evicting pod default/tomcat-deployment-c4745758d-w72bv
evicting pod ingress-nginx/nginx-ingress-controller-77db54fc46-6wkf4
pod/tomcat-deployment-c4745758d-w72bv evicted
pod/nginx-ingress-controller-77db54fc46-6wkf4 evicted
node/kubernetes-node3 evicted
...
kubernetes-node1    Ready                      <none>   5d23h   v1.18.4
kubernetes-node2    Ready                      <none>   5d23h   v1.18.4
kubernetes-node3    Ready,SchedulingDisabled   <none>   5d23h   v1.18.4
```

### 删除节点

```shell
root@kubernetes-master:/usr/local/kubernetes/service# kubectl delete node kubernetes-node3
# 输出内容
